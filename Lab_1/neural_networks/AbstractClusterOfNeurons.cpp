...
void AbstractClusterOfNeurons::initWeight() {
    for(int i = 0; i != _weightingFactors.rows(); ++i) {
        for(int j = 0; j != _weightingFactors.columns(); ++j) {
            _weightingFactors.setCell(i, j, getInitValue());
        }
    }
    for(int i = 0; i != _weightingShift.rows(); ++i) {
        for(int j = 0; j != _weightingShift.columns(); ++j) {
            _weightingShift.setCell(i, j, getInitValue());
        }
    }
}
