...
void SimpleClusterOfNeurons::summation() {
    ...
    if(!__inputs__.contains(this)) {
        ...
        Signal outputSignal(__outputSignal__.size(), __outputSignal__.threadCount());
        const auto &shiftVector = __weights__.__weightingShift__.getBaseRow();
        for(unsigned i = 0; i != shiftVector.size(); ++i) {
            for(int j = 0; j != outputSignal.threadCount(); ++j) {
                outputSignal.setSignal(shiftVector[i], i, j);
            }
        }

        int shift = 0;
        for(auto cluster : __inputs__) {
            if(outputSignal.threadCount() != cluster->threadsCount())
                throw std::exception();
            ...
            for(int i = 0; i != outputSignal.size(); ++i)  {
                for(int k = 0; k != inputSignal.size(); ++k) {
                    auto weight = __weights__.weight(i, k + shift);
                    for(int j = 0; j != outputSignal.threadCount(); ++j) {
                        auto value = outputSignal.signal(i, j);
                        value += weight * inputSignal.signal(k, j);
                        outputSignal.setSignal(value, i, j);
                    }
                }
            }
            shift += inputSignal.size();
